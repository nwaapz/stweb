...
# -*- coding: utf-8 -*-
"""
Script to update mobile header in all HTML files to match the specified format
"""
...
# The target mobile header (from user's specification with "پرشیا")
TARGET_HEADER = '''<header class="site__mobile-header">
...
def update_file(file_path):
    ...
    try:
        ...
        # Pattern to match the entire mobile header section
        pattern = r'<!--\s*site__mobile-header\s*-->.*?<header\s+class=["\']site__mobile-header["\']>.*?</header>\s*<!--\s*site__mobile-header\s*/\s*end\s*-->'
        
        match = re.search(pattern, content, re.DOTALL | re.IGNORECASE)
        
        if not match:
            # Try without comments
            pattern = r'<header\s+class=["\']site__mobile-header["\']>.*?</header>'
            match = re.search(pattern, content, re.DOTALL | re.IGNORECASE)
        ...
        if match:
            ...
            if is_minified:
                replacement = f'<!-- site__mobile-header -->{minify_html(TARGET_HEADER)}<!-- site__mobile-header / end -->'
            else:
...
def main():
    ...
    for f in sorted(html_files):
        if update_file(f):
            ...
            print(f"✗ {f.name} (no header found)")
