...
# -*- coding: utf-8 -*-
"""
Script to update mobile header in all HTML files to match index.html
"""
...
# The target mobile header (from user's specification)
TARGET_MOBILE_HEADER = '''<header class="site__mobile-header">
			...
			<div class="mobile-header">
				...
				<div class="container">
					...
					<div class="mobile-header__body"><button class="mobile-header__menu-button" type="button"><svg width="18px" height="14px">
							...
							</svg></button> <a class="mobile-header__logo" href="index.html"><img src="images/sttechLogo.png" alt="Logo" style="max-height: 52px;">
	...
	c-2.8,0-5.5-1.7-6.4-4C116.7,15.1,117.4,14,118.4,14z">
						...
						<div class="mobile-header__search mobile-search">
							...
							<form class="mobile-search__body"><input type="text" class="mobile-search__input" placeholder="Enter keyword or part number"> <button type="button" class="mobile-search__vehicle-picker" aria-label="انتخاب وسیله نقلیه"><svg width="20" height="20">
	...
	C15.5,10.2,14,11.3,14,12z"></path>
									...
									</svg> <span class="mobile-search__vehicle-picker-label">پرشیا</span></button> <button type="submit" class="mobile-search__button mobile-search__button--search"><svg width="20" height="20">
...
def minify_html(html):
    ...
    # Remove newlines and tabs, but keep single spaces
    html = re.sub(r'\s+', ' ', html)
    # Remove spaces around tags
    html = re.sub(r'>\s+<', '><', html)
    return html.strip()

def update_mobile_header_in_file(file_path):
    """Update mobile header in a single HTML file"""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Pattern to match the mobile header section - more flexible to handle minified HTML
        # Match from comment or header tag to closing header tag and optional comment
        pattern = r'(<!--\s*site__mobile-header\s*-->)?\s*<header\s+class=["\']site__mobile-header["\']>.*?</header>\s*(<!--\s*site__mobile-header\s*/\s*end\s*-->)?'
        
        # Find the existing mobile header
        match = re.search(pattern, content, re.DOTALL | re.IGNORECASE)
        
        if match:
            # Determine the correct href for the logo
            logo_href = 'index.html'  # All pages link back to index.html
            
            # Replace href in the target header
            target_header = TARGET_MOBILE_HEADER.replace('href="index.html"', f'href="{logo_href}"')
            
            # Check if the file is minified (no newlines in the header section)
            header_section = content[match.start():match.end()]
            is_minified = header_section.count('\n') < 3
            
            if is_minified:
                # Create minified version
                minified_header = minify_html(target_header)
                replacement = f'<!-- site__mobile-header -->{minified_header}<!-- site__mobile-header / end -->'
            else:
                # Use formatted version (match index.html style with tabs)
                replacement = f'<!-- site__mobile-header -->\n\t\t{target_header}\n\t\t<!-- site__mobile-header / end -->'
            ...
            print(f"  ⚠️  No mobile header found in {file_path}")
            return False
...
def main():
    ...
    # Exclude header variant files and mobile header variant files
    exclude_patterns = [
        'header-',
        'mobile-header-',
        'indexTest.html',
    ...
    html_files = [f for f in html_files if not any(pattern in f.name for pattern in exclude_patterns)]
    
    print(f"Found {len(html_files)} HTML files to process")
    print("=" * 60)
    
    if len(html_files) == 0:
        print("No HTML files found! Check the script directory.")
        return
    ...
    for html_file in sorted(html_files):
        print(f"Processing: {html_file.name}...", end=' ', flush=True)
        if update_mobile_header_in_file(html_file):
            print("✓ Updated")
            updated_count += 1
        else:
            print("✗ Skipped")
